...
const STAGES=[
 {ch:0,k:'Lease signed',ic:'key',title:'Folio Plus starts at signing.',
  benefit:'Sarah earns back on <b>rent and everyday life</b> from day one, in <b>Folio Credits</b>.',
  tech:'Activated by the AppFolio API. It lives in the resident portal she already uses.',
  merch:'20,000+ top merchants are live on day one.'},
 {ch:0,k:'Design your home',ic:'sofa',title:'A free 3D design before the keys.',
  ...
  tech:'Floor plans in AppFolio become 3D rooms. AI furnishes them, with walk-through and AR.',
  merch:'Suppliers sell direct, with no middlemen.'},
 {ch:0,k:'Welcome gift',ic:'gift',title:'A welcome gift, her way.',
  benefit:'<b>$50 from 800 Penn</b>, delivered as <b>50 Folio Credits</b>: an essentials kit or pizza night from a local spot.',
  tech:'The PMC sets it once in AppFolio, and every new lease gets it automatically.',
 ...
 {ch:0,k:'Move-in week',ic:'truck',title:'Movers and the TV mounted, handled.',
  ...
  tech:'Folio Space dispatches top-rated local pros, and the live concierge confirms each one.',
  merch:'Local pros get booked jobs with no ad spend.',human:true},
 {ch:1,k:'First rent',ic:'cash',title:'Rent that pays her back.',
  benefit:'On-time rent earns <b>1% back</b>, delivered as Folio Credits. 1% \u00d7 $2,105 = <b>$21.05</b>, above the <b>$20 monthly minimum</b>.',
  floor:'Every resident earns at least $20 a month in Folio Credits on rent, whatever the rent is.',
  tech:'The AppFolio payment event triggers the credit.',
  merch:'Folio Credits get spent at local merchants.'},
 {ch:1,k:'Card linking',ic:'card',title:'Link once. Earn everywhere.',
  benefit:'One tap links her Visa. Up to <b>5% back</b> in Folio Credits at Walmart, Kroger, CVS and more.',
  tech:'Visa card-linked offers. No new card, codes or receipts.',
  merch:'20,000+ top merchants reach her on the card she already carries.'},
 {ch:1,k:'Groceries',ic:'cart',title:'Earn on the weekly shop.',
  benefit:'Up to <b>5% back</b> in Folio Credits on the card she already has.',
  moment:['Folio Credit Drop','Double earn back at Kroger: her $86 shop earns $8.64.'],
 ...
 {ch:1,k:'Gas & commute',ic:'fuel',title:'Gas Boost week.',
  benefit:'Up to <b>25¢/gal back</b> in Folio Credits, with live prices nearby.',
 ...
 {ch:1,k:'Dining',ic:'fork',title:'240+ local spots, one map.',
  benefit:'Order nearby and <b>pay with Folio Credits</b>.',
  moment:['Folio Credit Drop','$5 off any local dinner tonight.'],
 ...
 {ch:1,k:'Pharmacy',ic:'pill',title:'CVS becomes her pharmacy.',
  benefit:'Pickup or delivery, auto refills and <b>2% back</b> in Folio Credits.',
 ...
 {ch:1,k:'Home services',ic:'spray',title:'A pro she knows by name.',
  ...
  tech:'Vetted-pro dispatch, live tracking and a quality check after every visit.',
 ...
 {ch:2,k:'Manager gifts',ic:'wrench',opt:true,title:'A $10 thank-you after every fix.',
  ...
  tech:'Triggered by AppFolio. The PMC sets it on or off and picks the amount.',
  merch:'Gets spent at the café downstairs.'},
 {ch:2,k:'Birthday',ic:'cake',title:'Dessert’s on us.',
  benefit:'<b>25 Folio Credits</b> arrive on the day.',
 ...
 {ch:2,k:'Renewal',ic:'heart',title:'Thanks for staying.',
  benefit:'A <b>$100 renewal gift</b> in Folio Credits, toward her housekeeper, Anna.',
  tech:'The PMC sets it once in AppFolio, and it goes out when she renews.',
  merch:'Gift money stays with trusted pros.'},
 {ch:2,k:'Moving out',ic:'box',title:'A great goodbye.',
  benefit:'Move-out is handled, and <b>her Folio Credits and membership move with her</b>.',
...
STAGES.forEach((s,i)=>{
  ...
  <article class="step" data-i="${i}" id="stage-${i+1}">
    ...
    <div class="pop">
      ...
      ${s.floor?`<div class="floor"><span class="fi">✓</span><p>${s.floor}</p></div>`:''}
      ${s.moment?`<div class="moment"><span class="mi">${ic('spark','#8073FF')}</span><div><small>${s.moment[0]}</small><p>${s.moment[1]}</p></div></div>`:''}
      <div class="how"><div class="t"><small><i></i>Tech</small><p>${s.tech}</p></div><div class="m"><small><i></i>Merchants</small><p>${s.merch}</p></div></div>
      ${s.human?`<div class="human"><span class="avs"><span style="background:var(--maddie)">M</span><span style="background:#0F97FF">J</span><span style="background:url(assets/pro-anna.jpg) center/cover"></span></span><span class="live"></span> Live concierge checks every booking</div>`:''}
    </div>
  </article>`);
});
...
/* audience */
const AUD={
 pmc:{cta:'Book with us',faq:'pmc',url:'#faq'},pms:{cta:'Learn more',faq:'pms',url:'#faq'},
 merchant:{cta:'Partner with us',faq:'merchant',url:'#faq'}
};
function setAud(a,scroll){
  document.querySelectorAll('.aud-chip').forEach(c=>c.setAttribute('aria-pressed',c.dataset.aud===a));
  document.querySelectorAll('[data-cta]').forEach(b=>{b.querySelector('.lbl').textContent=AUD[a].cta;b.dataset.aud=a;b.href=AUD[a].url});
  ...
  setTab(AUD[a].faq);
  try{localStorage.setItem('folio-aud',a)}catch(e){}
}
...
try{const s=localStorage.getItem('folio-aud');if(s&&AUD[s])setAud(s)}catch(e){}
...
/* confetti */
const cf=['#006DBF','#FFE448','#0F97FF','#22D195','#8073FF'];
